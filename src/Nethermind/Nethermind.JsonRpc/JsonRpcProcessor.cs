            ...
            do
            {
                ...
                else if (moveNext)
                {
                    ...
                    if (rpcRequest.Collection is not null)
                    {
                        ...
                        {
                            if (_logger.IsWarn) _logger.Warn($"The batch size limit was exceeded. The requested batch size {rpcRequest.Collection.Count}, and the current config setting is JsonRpc.{nameof(_jsonRpcConfig.MaxBatchSize)} = {_jsonRpcConfig.MaxBatchSize}. Url: {context.Url}");
